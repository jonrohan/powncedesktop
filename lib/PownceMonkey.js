...
PownceChat.prototype = {
	...
	loadFavorites: function() {
        ...
        try {
            ...
            selectStmt.text = "SELECT * FROM favorites ORDER BY id DESC";
	...
	uiPrepareNoteLink: function(note,min) {
		...
		var medlnk = document.createElement("a");
			...
			else if(note.link.url.match(/^http:\/\/www.digg.com\/$/)) {
				medobj.className = "media-object";
        ...
        switch (view) {
			...
			case "new":
				...
				switch(type){
					...
					break;
					case "event":
						$('filter-box').style.top = "242px";
						$('notes').style.top = "242px";
