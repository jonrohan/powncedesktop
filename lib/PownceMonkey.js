...
var properties = {
    api_urls: {
		...
		update_url: "http://dinnermint.org/PownceMonkey/release/current",
...
PownceChat.prototype = {
    initialize: function(options){
        ...
        this.getUser = function(username,limit) {
			...
			if(limit==0)
				var url = oInst.utilPrepareAPIURL("http://api.pownce.com/2.0/users/" + username + ".json?{app_key}");
            else
				var url = oInst.utilPrepareAPIURL("http://api.pownce.com/2.1/note_lists/" + username + ".json?{app_key}&limit=" + limit + "&filter=public");
    ...
    checkForUpdate: function(){
        ...
        new Ajax.Request("http://dinnermint.org/PownceMonkey/release/current", {
            onSuccess: handlerFunc,
            onFailure: errFunc
        });
    },
    ...
    getNewVersion: function(latest) {
        ...
        var handlerFunc = function(t) {
            ...
            if (result) {
                var urlString = "http://www.dinnermint.org/PownceMonkey/release/PownceMonkey-v" + latest + ".air";
        ...
        new Ajax.Request("http://dinnermint.org/PownceMonkey/release/" + latest, {
            onSuccess: handlerFunc,
            onFailure: errFunc
        });
    },
