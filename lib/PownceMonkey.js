...
PownceChat.prototype = {
	...
	uiPrepareNoteLink: function(note,min) {
		...
		var medlnk = document.createElement("a");
			...
			if(note.link.oembed) {
				if(note.link.oembed.type == "video") {
					...
					if (note.link.oembed.provider_name == "YouTube") {
						...
						if (note.link.oembed.title) {
							var medobjtitle = this.uiCreateMediaTitle(note.link.oembed.title);
						...
						if (!min) {
							...
							imglnk.className = "media-object-link";
							imglnk.addEventListener("click", function(event){
								chat.uiOpenVideoWindow("general", vidsrc);
		                    	chat.doMarkAsDownloaded(note.id);
								if (properties.settings.notes_collapsible) 
									chat.uiShowNoteDetails(note.id);
							});
				...
				medobj.className = "media-object";
				new MUXTAPE({muxurl:note.link.url,container:medobj})
			}
            medlnk.href = "#";
    ...
    loadSendToList: function(){
		...
		new Autocompleter.Local('send-to-autocomplete', 'send-to-list', ['ABBA', 'AC/DC', 'Aerosmith', 'America', 'Bay City Rollers', 'Black Sabbath', 'Boston', 'David Bowie', 'Can', 'The Carpenters', 'Chicago', 'The Commodores', 'Crass', 'Deep Purple', 'The Doobie Brothers', 'Eagles', 'Fleetwood Mac', 'Haciendo Punto en Otro Son', 'Heart', 'Iggy Pop and the Stooges', 'Journey', 'Judas Priest', 'KC and the Sunshine Band', 'Kiss', 'Kraftwerk', 'Led Zeppelin', 'Lindisfarne (band)', 'Lipps, Inc', 'Lynyrd Skynyrd', 'Pink Floyd', 'Queen', 'Ramones', 'REO Speedwagon', 'Rhythm Heritage', 'Rush', 'Sex Pistols', 'Slade', 'Steely Dan', 'Stillwater', 'Styx', 'Supertramp', 'Sweet', 'Three Dog Night', 'The Village People', 'Wings (fronted by former Beatle Paul McCartney)', 'Yes'], {});
    },
