...
PownceMonkey.prototype = {
        ...
        this.getFriends = function(){
            ...
            var handlerFunc = function(t){
                ...
                for (var i = 0; i < friends.friends.users.length; i++) {
					...
					oInstk.addPersonToDB(friends.friends.users[i]);
                }
				oInstk.loadPeoples();
        ...
        this.getFanof = function(){
            ...
            var handlerFunc = function(t){
                ...
                for (var i = 0; i < fan_of.fan_of.users.length; i++) {
					...
					oInstk.addPersonToDB(fan_of.fan_of.users[i]);
                }
				oInstk.loadPeoples();
    ...
    getLatestNotes : function() {
		this.uiSystemMessage("Checking for new notes");
        var oInst = this;
        ...
        var handlerFunc = function(t){
			...
			if (notes.notes.length != 0&&properties.pages.currentNotesPage==0) {
                ...
                oInst.uiSystemMessage(notes.notes.length + " new note" + ((notes.notes.length > 1) ? "s" : ""));
                oInst.uiNotify();
				...
				oInst.uiSystemMessage("No new notes");
			}
        };
        var errFunc = function(t){
        ...
        var url = this.utilPrepareAPIURL(properties.api_urls.note_list,options);
        new Ajax.Request(url, {
            onSuccess: handlerFunc,
            onFailure: errFunc,
            requestHeaders: ["Authorization", "Basic " + btoa(properties.user.username + ":" + properties.user.password)],
	...
	getNotes : function(){
		this.uiSystemMessage("Getting notes");
        var oInst = this;
        var handlerFunc = function(t){
			if(oInst.isError(t.responseText)) {
				...
				oInst.uiSystemMessage("Get Notes error: " + t.responseText,6);
				clearInterval(properties.interval.getnotes);
                oInst.uiSystemMessage("Ack! Server error, try again later.",5);
				return false;
			}
			...
			response = response.replace(/\\u00/gim, "\\\\u00");
            var notes = eval("(" + response + ")");
			oInst.uiSystemMessage("Notes received");
			oInst.getSendTo();
	        ...
	        if(oInst.getFriendCount()!=prof_count) {
				...
				oInst.getFriendCount();
                oInst.getFriends();
				if(properties.interval.getfriends)
					clearInterval(properties.interval.getfriends);
				properties.interval.getfriends = setInterval(oInst.getFriends, 3000);
				properties.pages.currentFanofPage = 0;
                oInst.getFanof();
                if(properties.interval.getfanof)
					clearInterval(properties.interval.getfanof);
	...
	getProfile: function(){
        ...
        var handlerFunc = function(t){
			...
			oInst.uiSystemMessage("Profile received");
            properties.user.profile = user;
            if (user.friend_request_count != 0) {
                oInst.uiAlertNewFriendRequest();
            }
        };
        var errFunc = function(t){
        ...
        var url = this.utilPrepareAPIURL(properties.api_urls.profile);
        new Ajax.Request(url, {
            onSuccess: handlerFunc,
            onFailure: errFunc,
            requestHeaders: ["Authorization", "Basic " + btoa(properties.user.username + ":" + properties.user.password)],
    ...
    getUser: function(username,limit) {
		...
		var url = "";
		if(limit==0)
			url = this.utilPrepareAPIURL("http://api.pownce.com/2.0/users/" + username + ".json?{app_key}");
        else
			url = this.utilPrepareAPIURL("http://api.pownce.com/2.1/note_lists/" + username + ".json?{app_key}&limit=" + limit);
		var headers = [];
	...
	loadFavorites: function() {
        ...
        try {
            ...
            try {
				...
				this.uiSystemMessage("trycatch error: " + error.message,6);
				conn.rollback();
                // error handling is shown in a subsequent code listing
				return false;
            }
	...
	loadPeoples: function() {
		this.uiSystemMessage("Loading People From DB ",6);
        var conn = this.getDBConnection();
		if(!conn)
			return null;
        // start a transaction
		...
		this.uiSystemMessage("Done Loading People From DB ",6);
		return true;
	},
    ...
    doDrop: function(e){
		...
		properties.ui.upload_file.addEventListener( air.IOErrorEvent.IO_ERROR, function(error) { chat.uiSystemMessage("trycatch error: " + e.message,6); });
    },
    ...
    doSaveProperties: function(){
		try {
			...
			this.uiSystemMessage("Error: " + e.message,6);
		}
    },
    ...
    doPostNewNote: function(){
        ...
        if (type == "message") {
			if(body==""||body=="post a note...") {
				this.uiSystemMessage("You must enter a message in the message box.",2);
				return;
			}
			...
			if(link==""||link=="http://") {
				this.uiSystemMessage("You must enter a url in the url box.",2);
				return;
			}
			if(body=="post a note...") {
				body = "";
			...
			if($('file-url').value=="") {
				this.uiSystemMessage("You must select a file to upload.",2);
				return;
			}
			...
			if(event_name==""||event_name=="What's Happening?") {
				this.uiSystemMessage("You must enter an event name.",2);
				return;
			}
			if(event_location==""||event_location=="Where at?") {
				this.uiSystemMessage("You must enter a location.",2);
				return;
			}
			if(date=="") {
				this.uiSystemMessage("You must enter a date.",2);
				return;
			}
			if(time=="") {
				this.uiSystemMessage("You must enter a time.",2);
				return;
			}
			if(body=="post a note...") {
				body = "";
    ...
    getFriendById: function(id){
		var user = null;
		this.uiSystemMessage("Getting Friend From DB ",6);
        var conn = this.getDBConnection();
		if(!conn)
			return null;
        // start a transaction
		...
		this.uiSystemMessage("Done Getting Friend From DB ",6);
		return user;
   },
    ...
    uiOpenImageWindow: function(type, imgsrc){
			...
			if(note.link.oembed) {
				if(note.link.oembed.type == "video") {
					...
					if(!min&&note.link.oembed.provider_name == "Flickr"||note.link.oembed.provider_name == "Zooomr"||note.link.oembed.provider_name == "static.zooomr.com") {
		                ...
		                imglnk.className = "media-object-link";
		                imglnk.addEventListener("click", function(event){
		                    chat.uiOpenImageWindow(note.link.oembed.provider_name, note.link.oembed.url);
		                    chat.doMarkAsDownloaded(note.id);
		                    if (properties.settings.notes_collapsible) 
		                        chat.uiShowNoteDetails(note.id);
		                });
	...
	uiPrepareNoteFile: function(note) {
        ...
        if (note.file.type == "image") {
            ...
            imglnk.addEventListener("click", function(event){
                chat.doNavigateToURL(note.file.url[0]);
                chat.doMarkAsDownloaded(note.id);
                if (properties.settings.notes_collapsible) 
                    chat.uiShowNoteDetails(note.id);
            });
	...
	addToolbarEvents: function() {
		...
		$('toolbar-clear-favorites').addEventListener('click', function(){
			if(chat.uiSystemMessage("Are you sure you want to delete all your favorites?",3))
				chat.deleteAllFavorites();
        });
		$('toolbar-refresh-people').addEventListener('click', function(){
			chat.deleteAllFavorites();
			properties.json.fan_of = [];
			properties.json.people = [];
			properties.pages.currentFriendPage = 0;
            chat.getFriends();
			if(properties.interval.getfriends)
				clearInterval(properties.interval.getfriends);
			properties.interval.getfriends = setInterval(chat.getFriends, 3000);
			properties.pages.currentFanofPage = 0;
            chat.getFanof();
            if(properties.interval.getfanof)
				clearInterval(properties.interval.getfanof);
			properties.interval.getfanof = setInterval(chat.getFanof, 3000);
        });
	},
	...
	uiSetProfileDetail: function(user) {
		...
		if(user.last_note_id) {
			...
			lastnotebody = this.uiPrepareNoteBody(lastnotebody);
        ...
        try {
            ...
            if (content) {
                ...
                if (props.settings) {
					...
					if(props.settings.hide_toolbar)
						properties.settings.hide_toolbar = props.settings.hide_toolbar;
					if(props.settings.toolbar_icon_text)
						properties.settings.toolbar_icon_text = props.settings.toolbar_icon_text;
					if(props.settings.transparency)
						properties.settings.transparency = props.settings.transparency;
					if(props.settings.x)
						properties.settings.x = props.settings.x;
					if(props.settings.y)
						properties.settings.y = props.settings.y;
					if(props.settings.width)
						properties.settings.width = props.settings.width;
					if(props.settings.height)
						properties.settings.height = props.settings.height;
					if(props.settings.hide_list)
						properties.settings.hide_list = props.settings.hide_list;
					if(props.settings.hide_offline)
						properties.settings.hide_offline = props.settings.hide_offline;
					if(props.settings.download_theme)
						properties.settings.download_theme = props.settings.download_theme;
					if(props.settings.font_size)
						properties.settings.font_size = props.settings.font_size;
					if(props.settings.friend_request_alert)
						properties.settings.friend_request_alert = props.settings.friend_request_alert;
					if(props.settings.play_sound)
						properties.settings.play_sound = props.settings.play_sound;
					if(props.settings.notes_collapsible)
						properties.settings.notes_collapsible = props.settings.notes_collapsible;
					if(props.settings.max_notes)
						properties.settings.max_notes = props.settings.max_notes;
					if(props.settings.minimize_to_tray)
						properties.settings.minimize_to_tray = props.settings.minimize_to_tray;
					if(props.settings.auto_login)
						properties.settings.auto_login = props.settings.auto_login;
					if(props.settings.remember_me)
						properties.settings.remember_me = props.settings.remember_me;
