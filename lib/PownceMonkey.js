...
PownceMonkey.prototype = {
    ...
    getUser: function(username,limit) {
		...
		var url = "";
		if(limit==0)
			url = PownceMonkey.prepareAPIURL(POWNCE_API_DOMAIN + "2.0/users/" + username + ".json?{app_key}");
        else
			url = PownceMonkey.prepareAPIURL(POWNCE_API_DOMAIN + POWNCE_API_VERSION + "/note_lists/" + username + ".json?{app_key}&limit=" + limit);
		var headers = [];
	...
	prepareNoteFile : function(note){
	    ...
	    if (note.file.type == "image") {
	        ...
	        imglnk.addEventListener("click", function(event){
	            PownceMonkey.doNavigateToURL(note.file.url);
	            chat.doMarkAsDownloaded(note.id);
	            if (properties.settings.notes_collapsible) 
	                chat.uiShowNoteDetails(note.id);
	        });
