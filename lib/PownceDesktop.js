...
PownceDesktop.prototype = {
	...
	cancelDialog: function(box,button) {
    ...
    doRemoveFriend: function(username){
		conf = this.ui.systemMessage({
			message: "Are you sure you want to unfriend " + username + "? This will remove all their notes from your page as well.",
			level: 3
		});
		if (conf) {
			this.ui.systemMessage({
    ...
    getNewVersion: function(latest) {
		var oInst = this;
        var handlerFunc = function(t) {
            var result = oInst.ui.systemMessage({
				message: "There is an update(v" + latest + ") available! Download?\n\n " + t.responseText,
				level: 3
			});
            if (result) {
    ...
    loadSendToList: function(){
		...
		if ("the public".match(properties.filter.sendtostr.toLowerCase())) {
			var pubau = this.ui.createAutocomplete("the public", "", POWNCE_API_DOMAIN + "profile_photos/p/o/w/pownce/4134_small.jpg", "public");
...
PownceDesktop.prototype.ui = {
	...
	addApplicationMenu: function() {
		...
		windowMenu.addSubmenu(helpSubmenu, "Help");		air.NativeApplication.nativeApplication.menu = windowMenu;
	},
