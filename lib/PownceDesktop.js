...
PownceDesktop.prototype = {
    ...
    initialize: function(options) {
        ...
        if (false&&properties.settings.auto_login) {
	...
	addToolbarEvents: function() {
		...
		$('toolbutton-preferences').addEventListener('click', function(){
    ...
    uiChangeView: function(view,type){
		...
		$('toolbutton-preferences').className = "toolbutton";
		...
		switch (view) {
            ...
            case "settings":
                this.ui.clearNewNote();
				$('toolbutton-preferences').className = "toolbutton selected-toolbutton";
				$('reply-button').disabled = false;
				$('id_stars').value = "";
		        $('current-rating').style.width = '0';
		        $('rsvp').selectedIndex = 0;
...
PownceDesktop.prototype.ui = {
	...
	prepareNoteFile : function(note){
	    ...
	    if (note.file.type == "image") {
			...
			medobj.innerHTML = "<embed width=\"200\" height=\"26\" flashvars=\"url=" + escape(note.file.direct_url) + "\" wmode=\"transparent\" quality=\"high\" name=\"audio0\" id=\"audio0\" style=\"margin-left:-20px;\" src=\"/lib/audioplayer.swf\" type=\"application/x-shockwave-flash\"/>";
	    }
	    var medlnk = document.createElement("a");
		...
		var medlnk = document.createElement("a");
			...
			if(note.oembed) {
				if(note.oembed.type == "video") {
					...
					/*if (note.oembed.provider_name == "YouTube") {
						...
						if (!min) {
							...
							imglnk.addEventListener("click", function(event){
								PownceDesktop.openVideoWindow("youtube", vidid);
								chat.doMarkAsDownloaded(note.id);
								if (properties.settings.notes_collapsible) 
									chat.uiShowNoteDetails(note.id);
							});
					...
					else if (note.oembed.provider_name == "qik") {
						var streamname = note.oembed.html.substring(note.oembed.html.indexOf("?streamname=") + 12, note.oembed.html.indexOf("&vid="));
						var vid = note.oembed.html.substring(note.oembed.html.indexOf("&vid=") + 5, note.oembed.html.indexOf("&playback="));
						var user = note.oembed.html.substring(note.oembed.html.indexOf("&user=") + 6, note.oembed.html.indexOf("&userlock="));
						note.oembed.streamname = streamname;
						if (note.oembed.title)
							medobj.appendChild(this.createMediaTitle(note.oembed.title));
						if (!min) {
							var medobj = document.createElement("div");
							...
							imglnk.className = "media-object-link";
							imglnk.addEventListener("click", function(event){
								PownceDesktop.openVideoWindow("qik", vid, streamname, user);
								chat.doMarkAsDownloaded(note.id);
								if (properties.settings.notes_collapsible) 
									chat.uiShowNoteDetails(note.id);
							});
							...
							var img = document.createElement("img");
							img.src = "http://qik.com/redir/" + note.oembed.streamname + ".jpg";
						...
						popout.className = "popout";
						popout.addEventListener("click", function(event){
		                    PownceDesktop.openVideoWindow("qik", vidid);
		                    chat.doMarkAsDownloaded(note.id);
		                    if (properties.settings.notes_collapsible) 
		                        chat.uiShowNoteDetails(note.id);
		                });
					...
					else if (note.oembed.provider_name == "Revision3") {
						var thumburl = note.oembed.html.substring(note.oembed.html.indexOf("Thumb=") + 6);
						thumburl = thumburl.substring(0,thumburl.indexOf("&amp;"));
						var vidsrc = note.oembed.html.substring(note.oembed.html.indexOf("value=\"") + 7);
						vidsrc = vidsrc.substring(0,vidsrc.indexOf("\""));
						vidsrc = vidsrc.replace(/&amp;/gim,"&");
						vidsrc = escape(vidsrc);
						var base = note.oembed.html.substring(note.oembed.html.indexOf("base=\"") + 6);
						...
						if (!min) {
							...
							imglnk.className = "media-object-link";
							imglnk.addEventListener("click", function(event){
								PownceDesktop.openVideoWindow("general", vidsrc, null, null, base);
		                    	chat.doMarkAsDownloaded(note.id);
								if (properties.settings.notes_collapsible) 
									chat.uiShowNoteDetails(note.id);
							});
					...
					else if (note.oembed.provider_name == "pownce") {
						var vidid = note.oembed.html.substring(note.oembed.html.indexOf("viddlerplayer-") + 14);
						vidid = vidid.substring(0,vidid.indexOf("\""));
						var vidsrc = note.oembed.html.substring(note.oembed.html.indexOf("value=\"") + 7);
