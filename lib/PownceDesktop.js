...
var PownceDesktop = new Class.create();
PownceDesktop.prototype = {
	...
	oauthAccessToken: function(){
		this.ui.systemMessage({
			message: "Getting OAuth Access Token"
		});
        var oInst = this;
        var successCallback = function(t){
			if(oInst.isError(t.responseText)) {
				oInst.ui.systemMessage({
					message: "OAuth Access Token error: " + t.responseText,
					level: 6
				});
				oInst.ui.systemMessage({
					message: "Ack! Server error, try again later.",
			...
			oInst.ui.systemMessage({
				message: "Got OAuth Access Token"
			});
            oInst.oauthVerify();
        };
		var failureCallback = function(t) {
			oInst.ui.systemMessage({
				message: "failure: " + t.responseText,
				level: 6
			});
   	...
   	oauthAuthorize: function(){
		this.ui.systemMessage({
			message: "OAuth Authorizing"
		});
        var oInst = this;
        var successCallback = function(t){
			if(oInst.isError(t.responseText)) {
				oInst.ui.systemMessage({
					message: "OAuth Authorizing error: " + t.responseText,
					level: 6
				});
				oInst.ui.systemMessage({
					message: "Ack! Server error, try again later.",
            	...
            	oInst.ui.systemMessage({
					message: "OAuth Authorization successfull"
				});
				...
				if(oInst.ui.systemMessage({
					message: "You must Authorize Pownce Monkey. Once authorized press OK to continue.",
					level: 3
				}))
					oInst.oauthAccessToken();
			}
        ...
        PownceDesktop.doNavigateToURL(url);
		//var postVars = url.split("?")[1];
		new Ajax.Request(url, {
            onSuccess: successCallback,
			onFailure: failureCallback,
    ...
    oauthInit: function(){
        this.ui.systemMessage({
			message: "OAuth Initializing"
		});
		var oInst = this;
        var successCallback = function(t){
			if(oInst.isError(t.responseText)) {
				oInst.ui.systemMessage({
					message: "OAuth Initializing error: " + t.responseText,
					level: 6
				});
				oInst.ui.systemMessage({
					message: "Ack! Server error, try again later.",
    ...
    oauthVerify: function(){
		this.ui.systemMessage({
			message: "OAuth Verifing"
		});
        var oInst = this;
        var successCallback = function(t){
			if(oInst.isError(t.responseText)) {
				oInst.ui.systemMessage({
					message: "OAuth Verify error: " + t.responseText,
					level: 6
				});
				oInst.ui.systemMessage({
					message: "Ack! Server error, try again later.",
        ...
        var url = OAuth.getUrl("get", POWNCE_API_DOMAIN + "auth/verify.json", properties.oauth.APP_KEY, properties.oauth.SECRET, properties.oauth.signature_method, properties.oauth.access_token, properties.oauth.access_token_secret, []);
        new Ajax.Request(url, {
            onSuccess: successCallback,
			onFailure: failureCallback,
            method: 'get'
	...
	addFavoriteToDB: function(note) {
        ...
        try {
            ...
            writeFiles.text = this.prepareSQL(properties.sql.insert_favorites,{
				...
				link_oembed_type : note.oembed.type,
				link_oembed_provider_name : note.oembed.provider_name,
				link_oembed_html : note.oembed.html,
				link_oembed_author_url : note.oembed.author_url,
				link_oembed_title : note.oembed.title,
				link_oembed_author_name : note.oembed.author_name,
				link_oembed_url : note.oembed.url,
	...
	loadFavorites: function() {
        ...
        try {
            ...
            try {
				...
				this.ui.systemMessage({
					message: "trycatch error: " + error.message,
					level: 6
				});
				conn.rollback();
                // error handling is shown in a subsequent code listing
	...
	prepareNoteForDB: function(note) {
		...
		if(note.file) {
			...
			if(!note.file.url)
				...
				note.file.url = note.file.url;
			if(!note.file.direct_url)
				note.file.direct_url = "";
			else
				note.file.direct_url = note.file.direct_url;
		}
		else {
			note.file = {
				"type" : "",
	...
	preparePersonForDB: function(person) {
		...
		if(!person.country)
			person.country = "";
		person.name = (person.last_name)?person.first_name + " " + person.last_name : person.short_name;
	...
	setPeopleRelationship: function(ids,rel) {
       var conn = this.getDBConnection();
		if(!conn)
			return null;
        // start a transaction
        ...
        try {
            ...
            var sql = this.prepareSQL(properties.sql.set_all_relationships,{
				relationship : rel
			});
			
			sql += " WHERE ";
			for(i=0;i<ids.length;i++) {
				sql += "id=" + ids[i] + ((i<ids.length-1)?" OR ":"");
			}
			selectStmt.text = sql;
	...
	doKeyboardShortcut: function(code) {
		switch(code) {
			...
			case 86: // v - view original note
				PownceDesktop.doNavigateToURL(properties.json.notes[properties.ui.current_scroll_note].permalink);
			break;
			case 76: // l - follow link url
				if(properties.json.notes[properties.ui.current_scroll_note].link)
				PownceDesktop.doNavigateToURL(properties.json.notes[properties.ui.current_scroll_note].link.url);
			break;
    ...
    doSaveProperties: function(){
		try {
	        ...
	        var props = "{version:" + VERSION + ",settings:{x:" + properties.settings.x + ",y:" + properties.settings.y + ",width:" + properties.settings.width + ",height:" + properties.settings.height + ",hide_list:" + properties.settings.hide_list + ",shuffle_song:" + properties.settings.shuffle_song + ",repeat_song:" + properties.settings.repeat_song + ",download_theme:" + properties.settings.download_theme + ",friend_request_alert:" + properties.settings.friend_request_alert + ",play_sound:" + properties.settings.play_sound + ",max_notes:" + properties.settings.max_notes + ", minimize_to_tray: " + properties.settings.minimize_to_tray + ",auto_login:" + properties.settings.auto_login + ",remember_me:" + properties.settings.remember_me + "}}";
	        var stream = new air.FileStream();
	        stream.open(file, air.FileMode.WRITE);
    ...
    getNewVersion: function(latest) {
		...
		var errFunc = function(t) {
			oInst.ui.systemMessage({
				message: "download release failure: " + t.responseText,
				level: 6
			});
			return;
		};
	...
	uiSetProfileDetail: function(user) {
		...
		//link.style.color = properties.ui.link_color;
		link.addEventListener("click", function() {
			PownceDesktop.doNavigateToURL(user.permalink);
		});
		...
		frlink.addEventListener("click", function() {
			PownceDesktop.doNavigateToURL("http://pownce.com/" + user.username + "/friends/");
		});
		...
		falink.addEventListener("click", function() {
			PownceDesktop.doNavigateToURL("http://pownce.com/" + user.username + "/fans/");
		});
		...
		faolink.addEventListener("click", function() {
			PownceDesktop.doNavigateToURL("http://pownce.com/" + user.username + "/fan_of/");
		});
	...
	uiSetNoteDetail: function(note) {
		...
		if (note.file) {
            ...
            alink.addEventListener("click", function(event){
                PownceDesktop.doNavigateToURL(note.file.url[0]);
            });
        }
    ...
    loadSendToList: function(){
		...
		this.setPeopleRelationship(ids,"friend");
		namearray.sort();
	...
	readXMLTemplate: function(prefsXMLstr) {
		try {
			...
			this.ui.systemMessage({
				message: "Loading Template error: " + error.message + " \nxmlstr: " + prefsXMLstr,
				level: 6
			});
		}
	},
	...
	saveTemplate: function() {
		var name = this.ui.systemMessage({
			message: "What would you like to call this template?",
			level: 8
		});
		if(this.getTemplateByName(name)) {
			if (!this.ui.systemMessage({
				message: "A template of that name already exists, would you like to overwrite it?",
				level: 3
			})) {
				this.ui.systemMessage({
					message: "Save template canceled"
				});
				return;
			}
			else {
...
PownceDesktop.doNavigateToURL = function(url){
    var urlReq = new air.URLRequest(url);
...
PownceDesktop.prototype.ui = {
	...
	addApplicationMenu: function() {
		...
		helpMenuItem.addEventListener(air.Event.SELECT, function() {
			PownceDesktop.doNavigateToURL("https://pownce.pbwiki.com/Pownce-Desktop");
		});
		...
		air.NativeApplication.nativeApplication.menu = windowMenu;
	},
	...
	alertNewFriendRequest: function(){
        if (properties.settings.friend_request_alert) {
            ...
            var res = this.systemMessage({
				message: "You have " + num + " new friend requests. Accept or deny now?",
				level: 3
			});
            if (res) {
                PownceDesktop.doNavigateToURL("http://pownce.com/friend_requests/");
            }
        }
    },
    ...
    createMediaCredits: function(type, author, link, note_id){
        ...
        authlink.addEventListener("click", function(event){
            PownceDesktop.doNavigateToURL(link);
       });
	    ...
	    for (l = 0; l < links.length; l++) {
	        links[l].addEventListener("click", function(event){
	            ...
	            if (urllnk.indexOf("!") == 0) 
	                ...
	                PownceDesktop.doNavigateToURL(urllnk);
	        });
	    }
	    return bdy;
	},
	...
	prepareNoteFile : function(note){
	    ...
	    if (note.file.type == "image") {
	        ...
	        imglnk.addEventListener("click", function(event){
	            PownceDesktop.doNavigateToURL(note.file.url);
	            chat.doMarkAsDownloaded(note.id);
	        });
	        var img = document.createElement("img");
        ...
        meda.innerHTML = shorturl.substring(0, 25) + ((shorturl.length > 25) ? "..." : "");
			...
			if(note.oembed) {
				if(note.oembed.type == "video") {
					...
					if (!min) {
						...
						imglnk.innerHTML = note.oembed.html.replace("<embed","<param name=\"wmode\" value=\"transparent\"/><embed wmode=\"transparent\"");
						medobj.appendChild(imglnk);
		            ...
		            if(!min&&note.oembed.provider_name == "Flickr"||note.oembed.provider_name == "Zooomr"||note.oembed.provider_name == "static.zooomr.com") {
						var medobj = document.createElement("div");
						medobj.className = "media-object";
						if (note.oembed.title)
							medobj.appendChild(this.createMediaTitle(note.oembed.title));
		                ...
		                imglnk.className = "media-object-link";
		                imglnk.addEventListener("click", function(event){
		                    PownceDesktop.openImageWindow(note.oembed.provider_name, note.oembed.url);
		                    chat.doMarkAsDownloaded(note.id);
		                });
		                var img = document.createElement("img");
		                ...
		                imglnk.className = "media-object-link";
		                imglnk.addEventListener("click", function(event){
		                    PownceDesktop.doNavigateToURL(note.link.url);
		                    chat.doMarkAsDownloaded(note.id);
		                });
		                var img = document.createElement("img");
