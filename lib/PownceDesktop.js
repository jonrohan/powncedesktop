...
PownceDesktop.DB = function(){
    ...
    this.connect = function(){
		...
		air.trace("End DB connect");
    };
    this.query = function(sqlStatement, options){
		
		air.trace("Start DB query");
        var result = null;
        try {
            ...
            try {
				...
				air.trace("Message: " + error.message);
		...
		air.trace("End DB query");
        return result;
    };
    this.commit = function(){
		air.trace("Start DB commit");
		try {
		...
		air.trace("End DB commit");
    };
	this.prepareSQL = function(query, tokens) {
		air.trace("Start DB prepareSQL");
		for( var token in tokens) {
	        query = query.replace("{" + token + "}", tokens[token]);
		}
		air.trace("End DB prepareSQL");
		return query;
	};
}
